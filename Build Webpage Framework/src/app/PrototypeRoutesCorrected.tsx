...
function BusinessArchitectureScreen({ session, workspace, architecture, setArchitecture, onSignOut }: { session: AuthSession | null; workspace: Workspace | null; architecture: BusinessArchitecture | null; setArchitecture: (architecture: BusinessArchitecture) => void; onSignOut: () => void }) {
  const [form, setForm] = useState({ name: architecture?.name || (workspace ? `Business Architecture for ${getWorkspaceName(workspace)}` : ''), description: architecture?.description || '', currentStateSummary: architecture?.currentStateSummary || '', futureStateSummary: architecture?.futureStateSummary || '', organizationUnits: architecture?.organizationUnits || '', existingValueStreams: architecture?.existingValueStreams || '', businessCapabilities: architecture?.businessCapabilities || '', businessProcesses: architecture?.businessProcesses || '', stakeholdersPersonas: architecture?.stakeholdersPersonas || '', informationConcepts: architecture?.informationConcepts || '', businessImpacts: architecture?.businessImpacts || '', status: architecture?.status || 'draft' });
  const updateField = (field: keyof typeof form, value: string) => setForm((current) => ({ ...current, [field]: value }));
  ...
  const submitArchitecture = (event: FormEvent) => {
    ...
    if (!workspace) return;
    const now = new Date().toISOString();
    const nextArchitecture: BusinessArchitecture = { id: architecture?.id || createId('business-architecture'), workspaceId: workspace.id, name: form.name.trim() || `Business Architecture for ${getWorkspaceName(workspace)}`, description: form.description, currentStateSummary: form.currentStateSummary, futureStateSummary: form.futureStateSummary, organizationUnits: form.organizationUnits, existingValueStreams: form.existingValueStreams, businessCapabilities: form.businessCapabilities, businessProcesses: form.businessProcesses, stakeholdersPersonas: form.stakeholdersPersonas, informationConcepts: form.informationConcepts, businessImpacts: form.businessImpacts, status: form.status || 'draft', createdAt: architecture?.createdAt || now, updatedAt: now };
        ...
        <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100">
          <CardContent className="pt-6">
            <form className="grid gap-5" onSubmit={submitArchitecture}>
              ...
              <div className="grid gap-5 md:grid-cols-2"><Field id="current-state" label="Current State Summary"><Textarea id="current-state" value={form.currentStateSummary} onChange={(event) => updateField('currentStateSummary', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="future-state" label="Future State Summary"><Textarea id="future-state" value={form.futureStateSummary} onChange={(event) => updateField('futureStateSummary', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field></div>
              <div className="grid gap-5 md:grid-cols-2"><Field id="org-units" label="Organization Units / Teams"><Textarea id="org-units" value={form.organizationUnits} onChange={(event) => updateField('organizationUnits', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="value-streams" label="Existing Value Streams"><Textarea id="value-streams" value={form.existingValueStreams} onChange={(event) => updateField('existingValueStreams', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="capabilities" label="Business Capabilities"><Textarea id="capabilities" value={form.businessCapabilities} onChange={(event) => updateField('businessCapabilities', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="processes" label="Business Processes"><Textarea id="processes" value={form.businessProcesses} onChange={(event) => updateField('businessProcesses', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="stakeholders" label="Stakeholders / Personas"><Textarea id="stakeholders" value={form.stakeholdersPersonas} onChange={(event) => updateField('stakeholdersPersonas', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field><Field id="information-concepts" label="Information / Data Concepts"><Textarea id="information-concepts" value={form.informationConcepts} onChange={(event) => updateField('informationConcepts', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" /></Field></div>
...
export default function PrototypeRoutesCorrected() {
