...
function KeyActivitiesScreen() {
  ...
  return (
    <PrototypeShell session={session} workspace={workspace}>
      <section className="space-y-8">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><Badge className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">Value stream detail</Badge><h1 className="retro-heading mt-4 text-3xl text-cyan-300">Key Activities</h1><p className="mt-3 max-w-3xl text-slate-300">Key Activities belong to Value Streams and describe where value is created, transferred, delayed, blocked, or improved.</p></div><Button variant="outline" onClick={() => navigateTo('/dashboard')} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">Back to Dashboard</Button></div>
          ...
          <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100">
            ...
            <CardContent className="space-y-5">
              ...
              <form className="grid gap-5" onSubmit={saveActivity}>
                ...
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={resetForm} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">Clear</Button><Button type="submit" disabled={valueStreams.length === 0} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400 disabled:opacity-50"><Save className="mr-2 h-4 w-4" />{editingId ? 'Update Key Activity' : 'Save Key Activity'}</Button></div>
              </form>
            </CardContent>
          </Card>
