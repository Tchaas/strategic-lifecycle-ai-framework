...
import BasePrototypeRoutes from './PrototypeRoutesCapabilityImpact';
...
type StrategicObjective = { id?: string; workspaceId?: string; strategicInitiativeName?: string; status?: string };
type BusinessArchitecture = { id?: string; workspaceId?: string; strategicObjectiveId?: string; name?: string; status?: string };
...
const asArray = <T,>(value: T[] | T | null | undefined): T[] => Array.isArray(value) ? value : value ? [value] : [];
...
const migratePrototypeData = () => {
  const workspace = loadWorkspace();
  if (!workspace?.id) return;

  const objectiveRecords = loadJson<StrategicObjective[]>(strategicObjectivesStorageKey, []);
  const normalizedObjectives = objectiveRecords.map((objective) => {
    if (!objective.workspaceId && (objective.id || objective.strategicInitiativeName)) return { ...objective, workspaceId: workspace.id };
    return objective;
  });
  if (JSON.stringify(objectiveRecords) !== JSON.stringify(normalizedObjectives)) saveJson(strategicObjectivesStorageKey, normalizedObjectives);

  const objectiveId = normalizedObjectives.find((objective) => objective.workspaceId === workspace.id && objective.id)?.id;
  const rawArchitecture = loadJson<BusinessArchitecture[] | BusinessArchitecture | null>(businessArchitectureStorageKey, []);
  const architectureRecords = asArray(rawArchitecture);
  const normalizedArchitectures = architectureRecords.map((architecture) => ({
    ...architecture,
    workspaceId: architecture.workspaceId || workspace.id,
    strategicObjectiveId: architecture.strategicObjectiveId || objectiveId || '',
  }));
  if (JSON.stringify(architectureRecords) !== JSON.stringify(normalizedArchitectures)) saveJson(businessArchitectureStorageKey, normalizedArchitectures);

  const architectureId = normalizedArchitectures.find((architecture) => architecture.workspaceId === workspace.id && architecture.id)?.id;
  const streamRecords = loadJson<ValueStream[]>(valueStreamsStorageKey, []);
  const normalizedStreams = streamRecords.map((stream) => ({
    ...stream,
    workspaceId: stream.workspaceId || workspace.id,
    businessArchitectureId: stream.businessArchitectureId || architectureId || '',
  }));
  if (JSON.stringify(streamRecords) !== JSON.stringify(normalizedStreams)) saveJson(valueStreamsStorageKey, normalizedStreams);
};

const loadObjectives = (workspaceId?: string) => workspaceId ? loadJson<StrategicObjective[]>(strategicObjectivesStorageKey, []).filter((item) => item.workspaceId === workspaceId && (item.id || item.strategicInitiativeName)) : [];
const loadBusinessArchitectures = (workspaceId?: string) => {
  ...
  return asArray(raw).filter((item) => item.workspaceId === workspaceId);
};
...
const persistWorkspaceInvites = (workspaceId: string, workspaceInvites: WorkspaceInvite[]) => {
  ...
  saveJson(invitesStorageKey, [...otherInvites, ...workspaceInvites]);
};
...
function SummaryTile({ label, value, tone = 'text-cyan-300' }: { label: string; value: ReactNode; tone?: string }) {
  return <div className="min-w-0 rounded-md border border-slate-700 bg-slate-950 p-4"><div className="break-words text-xs uppercase text-slate-500">{label}</div><div className={`mt-1 break-words text-2xl font-semibold ${tone}`}>{value}</div></div>;
}

function DashboardScreen() {
  migratePrototypeData();
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('storage', reload); window.removeEventListener('hashchange', reload); };
  }, []);
  ...
  const canOpenBusinessArchitecture = !!workspace && (objectives.length > 0 || architectures.length > 0);
    ...
    <PrototypeShell session={session} workspace={workspace}>
      <section className="space-y-8">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,480px)]">
          <Card className="min-w-0 rounded-md border-cyan-500/50 bg-slate-900 text-slate-100"><CardHeader><Badge className="w-fit rounded-sm bg-cyan-500 text-black hover:bg-cyan-400">Frontend-only prototype</Badge><CardTitle className="retro-heading pt-3 text-2xl text-cyan-300">Welcome to AI Augmented Lifecycle framework</CardTitle><CardDescription className="text-slate-300">Create the company workspace, define strategic objectives, capture business architecture, then define value streams, key activities, capabilities, and business impacts.</CardDescription></CardHeader><CardContent className="space-y-5"><PrototypeNotice /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><SummaryTile label="Workspace" value={workspace ? getWorkspaceName(workspace) : 'Not created'} tone="text-slate-100" /><SummaryTile label="Objectives" value={`${objectives.length} / ${objectiveLimit}`} tone="text-lime-300" /><SummaryTile label="Architecture" value={`${architectures.length} / ${architectureLimit}`} /><SummaryTile label="Value Streams" value={valueStreams.length} /><SummaryTile label="Capabilities" value={capabilities.length} tone="text-lime-300" /><SummaryTile label="Impacts" value={impacts.length} tone="text-fuchsia-300" /></div></CardContent></Card>
        ...
        <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
          ...
          <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100"><CardHeader><div className="flex items-start gap-3"><Workflow className="mt-1 h-5 w-5 text-cyan-300" /><div><CardTitle className="retro-heading text-cyan-300">Business Architecture</CardTitle><CardDescription className="text-slate-300">{architectures.length} structure{architectures.length === 1 ? '' : 's'}, {valueStreams.length} value stream{valueStreams.length === 1 ? '' : 's'}</CardDescription></div></div></CardHeader><CardContent className="space-y-4 text-sm text-slate-300"><p>Capture organization structures and value streams that downstream analysis will support.</p><Button disabled={!canOpenBusinessArchitecture} onClick={() => navigateTo('/business-architecture')} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400 disabled:opacity-50">Build Business Architecture</Button>{workspace && objectives.length === 0 && architectures.length === 0 ? <p className="text-xs text-yellow-200">Create a Strategic Objective first.</p> : null}</CardContent></Card>
...
export default function PrototypeRoutesDashboardStability() {
  migratePrototypeData();
  const [route, setRoute] = useState(() => getRoute());
  useEffect(() => { const handleHashChange = () => { migratePrototypeData(); setRoute(getRoute()); }; window.addEventListener('hashchange', handleHashChange); return () => window.removeEventListener('hashchange', handleHashChange); }, []);
