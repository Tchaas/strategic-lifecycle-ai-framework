...
import { ArrowLeft, Building2, Check, Database, GitBranch, KeyRound, LayoutDashboard, LogIn, LogOut, Save, Shield, UserPlus, Workflow } from 'lucide-react';
...
const currentPrototypeFlow = [
  'Sign in or sign up',
  'Create company workspace',
  'Capture business architecture details',
  'Strategic objectives coming next',
];

const createId = (prefix: string) => `${prefix}-${Date.now()}-${Math.random().toString(16).slice(2)}`;

const navigateTo = (route: PrototypeRoute | '/') => {
        ...
        <nav className="flex flex-wrap items-center gap-2 text-sm">
          ...
          {session && (
            <>
              ...
              {workspace && (
                <Button variant="ghost" className="rounded-sm text-slate-200 hover:bg-slate-800 hover:text-white" onClick={() => navigateTo('/business-architecture')}>
                  Business Architecture
                </Button>
              )}
...
function AuthScreen({ mode, session, workspace, onSignIn, onSignOut }: { mode: 'login' | 'signup'; session: AuthSession | null; workspace: Workspace | null; onSignIn: (session: AuthSession) => void; onSignOut: () => void }) {
  ...
  const continueAfterAuth = (provider: AuthSession['provider']) => {
    const nextSession = {
      email: email.trim() || 'prototype.user@example.com',
      provider,
        ...
        <section className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div className="space-y-5">
            ...
            <p className="max-w-2xl text-lg leading-relaxed text-slate-300">
              This screen represents the expected Google handoff before entering the workspace flow.
            </p>
            <PrototypeNotice />
          </div>
          ...
          <Card className="rounded-md border-cyan-500/60 bg-slate-900 text-slate-100 shadow-[0_0_30px_rgba(34,211,238,0.15)]">
            ...
            <CardContent className="space-y-4">
              <button
                type="button"
                onClick={() => continueAfterAuth('google')}
    ...
    <PrototypeShell session={session} workspace={workspace} onSignOut={onSignOut}>
      <section className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
        <div className="space-y-5">
          ...
          <p className="max-w-2xl text-lg leading-relaxed text-slate-300">
            Sign in to access the company workspace and begin capturing business architecture context for the lifecycle framework.
          </p>
            ...
            <form className="space-y-4" onSubmit={submitRecovery}>
              ...
              {submitted && (
                <div className="rounded-md border border-lime-500/40 bg-lime-400/10 p-4 text-sm text-lime-100">
                  Mock recovery submitted. A real reset email will be implemented when backend authentication is added.
                </div>
              )}
          ...
          <CardContent className="pt-6">
            <form className="grid gap-5" onSubmit={submitWorkspace}>
              <div className="grid gap-5 md:grid-cols-2">
                <Field id="company-name" label="Company / Workspace Name">
                  <Input id="company-name" value={form.companyName} onChange={(event) => updateField('companyName', event.target.value)} className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="legal-name" label="Legal Name">
                  <Input id="legal-name" value={form.legalName} onChange={(event) => updateField('legalName', event.target.value)} className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <Field id="description" label="Company Profile Description">
                <Textarea id="description" value={form.description} onChange={(event) => updateField('description', event.target.value)} className="min-h-24 border-slate-700 bg-slate-950 text-slate-100" />
              </Field>
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Field id="operating-model" label="Operating Model">
                  <Input id="operating-model" value={form.operatingModel} onChange={(event) => updateField('operatingModel', event.target.value)} placeholder="Centralized, federated, product-led, shared services" className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="business-model" label="Business Model">
                  <Input id="business-model" value={form.businessModel} onChange={(event) => updateField('businessModel', event.target.value)} placeholder="B2B, B2C, marketplace, logistics, platform" className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <Field id="customers" label="Primary Customers / Users">
                  <Textarea id="customers" value={form.primaryCustomers} onChange={(event) => updateField('primaryCustomers', event.target.value)} className="min-h-24 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="products-services" label="Primary Products / Services">
                  <Textarea id="products-services" value={form.primaryProductsServices} onChange={(event) => updateField('primaryProductsServices', event.target.value)} className="min-h-24 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <Field id="strategic-context" label="Strategic Context">
                <Textarea id="strategic-context" value={form.strategicContext} onChange={(event) => updateField('strategicContext', event.target.value)} className="min-h-24 border-slate-700 bg-slate-950 text-slate-100" />
              </Field>
              <Field id="admin-email" label="Default Admin Email">
                <Input id="admin-email" type="email" value={form.defaultAdminEmail} onChange={(event) => updateField('defaultAdminEmail', event.target.value)} className="border-slate-700 bg-slate-950 text-slate-100" />
              </Field>
      ...
      <section className="space-y-8">
        <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
          <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100">
            <CardHeader>
              ...
              <CardDescription className="text-slate-300">
                Start with the company workspace, then capture business architecture details before we add strategic objective creation in the next iteration.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-4">
                {currentPrototypeFlow.map((step, index) => (
          ...
          <Card className="rounded-md border-fuchsia-500/50 bg-slate-900 text-slate-100">
            ...
            <CardContent className="space-y-3 text-sm text-slate-300">
              ...
              <Button variant="outline" onClick={onSignOut} className="w-full rounded-sm border-fuchsia-500 bg-slate-950 text-fuchsia-200 hover:bg-fuchsia-500 hover:text-white">
                <LogOut className="mr-2 h-4 w-4" />
                Sign Out
              </Button>
            </CardContent>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100">
            <CardHeader>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <CardTitle className="retro-heading text-cyan-300">Workspace / Company Profile</CardTitle>
                  <CardDescription className="text-slate-300">{workspace ? getWorkspaceName(workspace) : 'No workspace created yet'}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-slate-300">
              {workspace ? (
                <>
                  <p>{workspace.description || 'No company description entered.'}</p>
                  <dl className="grid gap-3 sm:grid-cols-2">
                    <div><dt className="text-xs uppercase text-slate-500">Business Unit</dt><dd className="text-slate-100">{workspace.businessUnit || 'Not set'}</dd></div>
                    <div><dt className="text-xs uppercase text-slate-500">Industry</dt><dd className="text-slate-100">{workspace.industry || 'Not set'}</dd></div>
                    <div><dt className="text-xs uppercase text-slate-500">Operating Model</dt><dd className="text-slate-100">{workspace.operatingModel || 'Not set'}</dd></div>
                    <div><dt className="text-xs uppercase text-slate-500">Business Model</dt><dd className="text-slate-100">{workspace.businessModel || 'Not set'}</dd></div>
                  </dl>
                </>
              ) : (
                <p>Create the company profile workspace before entering business architecture details.</p>
              )}
              <Button onClick={() => navigateTo('/workspace-onboarding')} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">
                {workspace ? 'View / Edit Workspace' : 'Create Workspace'}
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-md border-lime-500/50 bg-slate-900 text-slate-100">
            <CardHeader>
              <div className="flex items-start gap-3">
                <Workflow className="mt-1 h-5 w-5 text-lime-300" />
                <div>
                  <CardTitle className="retro-heading text-lime-300">Business Architecture Details</CardTitle>
                  <CardDescription className="text-slate-300">{architecture ? architecture.name || 'Business architecture draft' : 'Ready after workspace creation'}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-slate-300">
              <p>
                Capture existing value streams, capabilities, processes, stakeholders, data concepts, and business impacts that may later support strategic objectives.
              </p>
              {architecture && (
                <dl className="grid gap-3 sm:grid-cols-2">
                  <div><dt className="text-xs uppercase text-slate-500">Status</dt><dd className="text-slate-100">{architecture.status}</dd></div>
                  <div><dt className="text-xs uppercase text-slate-500">Value Streams</dt><dd className="text-slate-100">{architecture.existingValueStreams ? 'Captured' : 'Not set'}</dd></div>
                  <div><dt className="text-xs uppercase text-slate-500">Capabilities</dt><dd className="text-slate-100">{architecture.businessCapabilities ? 'Captured' : 'Not set'}</dd></div>
                  <div><dt className="text-xs uppercase text-slate-500">Impacts</dt><dd className="text-slate-100">{architecture.businessImpacts ? 'Captured' : 'Not set'}</dd></div>
                </dl>
              )}
              <Button disabled={!workspace} onClick={() => navigateTo('/business-architecture')} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400 disabled:opacity-50">
                {architecture ? 'View / Edit Business Architecture' : 'Create Business Architecture Details'}
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="rounded-md border-cyan-500/50 bg-slate-900 text-slate-100">
          <CardHeader>
            ...
            <CardDescription className="text-slate-300">This is the mapped schema/API flow for later backend integration.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-7">
              {lifecycleFlow.map((step, index) => (
                <div key={step} className={`rounded-md border p-3 text-center text-xs ${index === 0 || index === 2 ? 'border-lime-400 bg-lime-400/15 text-lime-100' : 'border-slate-700 bg-slate-950 text-slate-300'}`}>
                  <div className="mb-1 font-semibold">{index + 1}</div>
                  {step}
                </div>
              ))}
      ...
      <PrototypeShell session={session} workspace={workspace} onSignOut={onSignOut}>
        <section className="mx-auto max-w-3xl space-y-6">
          <Card className="rounded-md border-yellow-400/60 bg-slate-900 text-slate-100">
            ...
            <CardContent>
              <Button onClick={() => navigateTo('/workspace-onboarding')} className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">Create Workspace</Button>
            </CardContent>
          </Card>
        </section>
      </PrototypeShell>
  ...
  return (
    <PrototypeShell session={session} workspace={workspace} onSignOut={onSignOut}>
      <section className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
          <div>
            <Badge className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">Business architecture</Badge>
          ...
          <CardContent className="pt-6">
            <form className="grid gap-5" onSubmit={submitArchitecture}>
              <div className="grid gap-5 md:grid-cols-[1fr_220px]">
                <Field id="ba-name" label="Business Architecture Name">
                  <Input id="ba-name" value={form.name} onChange={(event) => updateField('name', event.target.value)} className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="ba-status" label="Status">
                  <Input id="ba-status" value={form.status} onChange={(event) => updateField('status', event.target.value)} placeholder="draft, active, archived" className="border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <Field id="ba-description" label="Description">
                <Textarea id="ba-description" value={form.description} onChange={(event) => updateField('description', event.target.value)} className="min-h-24 border-slate-700 bg-slate-950 text-slate-100" />
              </Field>
              <div className="grid gap-5 md:grid-cols-2">
                <Field id="current-state" label="Current State Summary">
                  <Textarea id="current-state" value={form.currentStateSummary} onChange={(event) => updateField('currentStateSummary', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="future-state" label="Future State Summary">
                  <Textarea id="future-state" value={form.futureStateSummary} onChange={(event) => updateField('futureStateSummary', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <Field id="org-units" label="Organization Units / Teams">
                  <Textarea id="org-units" value={form.organizationUnits} onChange={(event) => updateField('organizationUnits', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="value-streams" label="Existing Value Streams">
                  <Textarea id="value-streams" value={form.existingValueStreams} onChange={(event) => updateField('existingValueStreams', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="capabilities" label="Business Capabilities">
                  <Textarea id="capabilities" value={form.businessCapabilities} onChange={(event) => updateField('businessCapabilities', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="processes" label="Business Processes">
                  <Textarea id="processes" value={form.businessProcesses} onChange={(event) => updateField('businessProcesses', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="stakeholders" label="Stakeholders / Personas">
                  <Textarea id="stakeholders" value={form.stakeholdersPersonas} onChange={(event) => updateField('stakeholdersPersonas', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
                <Field id="information-concepts" label="Information / Data Concepts">
                  <Textarea id="information-concepts" value={form.informationConcepts} onChange={(event) => updateField('informationConcepts', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
                </Field>
              </div>
              <Field id="business-impacts" label="Business Impacts / Gaps">
                <Textarea id="business-impacts" value={form.businessImpacts} onChange={(event) => updateField('businessImpacts', event.target.value)} className="min-h-28 border-slate-700 bg-slate-950 text-slate-100" />
              </Field>
              <div className="rounded-md border border-fuchsia-500/40 bg-fuchsia-400/10 p-4 text-sm text-fuchsia-100">
                Strategic objectives remain intentionally out of this first source update. The next iteration can connect this business architecture context to objective creation without replacing the legacy content.
              </div>
              <div className="flex justify-end">
                <Button type="submit" className="rounded-sm bg-lime-500 text-black hover:bg-lime-400">
                  <Save className="mr-2 h-4 w-4" />
                  Save Business Architecture
                </Button>
              </div>
            </form>
          </CardContent>
...
export default function PrototypeRoutes() {
  ...
  useEffect(() => {
    setArchitecture(loadBusinessArchitecture(workspace?.id));
  }, [workspace?.id]);
  ...
  if (route === '/login') {
    return <AuthScreen mode="login" session={session} workspace={workspace} onSignIn={setSession} onSignOut={signOut} />;
  }

  if (route === '/signup') {
    return <AuthScreen mode="signup" session={session} workspace={workspace} onSignIn={setSession} onSignOut={signOut} />;
  }

  if (route === '/forgot-password') {
    return <ForgotPasswordScreen session={session} workspace={workspace} onSignOut={signOut} />;
  }

  if (route === '/workspace-onboarding') {
    return <WorkspaceScreen session={session} workspace={workspace} setWorkspace={setWorkspace} onSignOut={signOut} />;
  }

  if (route === '/business-architecture') {
    return <BusinessArchitectureScreen session={session} workspace={workspace} architecture={architecture} setArchitecture={setArchitecture} onSignOut={signOut} />;
  }
